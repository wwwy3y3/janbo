...
acc_differ = 1
num_selec_attri = 0
candidate_attribute = [i for i in xrange(1,10)]#record the candidate attribute
select_attributes = []
accuracies = [0]#let selected attribute1's acc_differ>0
while acc_differ > 0:
    corrects = [] #of correct for each attribute
    for k in candidate_attribute:#attribute1 to 9 
        predict_class =[] # predict class value for test 
        for i in range(0,len(test)):
            v = int(test[i][k]) #use the attribute k
            posteriors = [] # posterior for each class
            for j in range(1,8): #class 1 to 7
                prior = count_class[j-1]/float(len(train))
                likelihood = (count_likes[k-1][j-1][v-1]+1)/(count_class[j-1]+10)
                likelihoods.append(likelihood)
                cumu_like = 1
                for item in likelihoods:
                    cumu_like *= item
                posterior = prior * cumu_like
    ...
    for i in range(0,len(corrects)):#find which attribute should be selected
        if corrects[i] == max(corrects):
            select_attribute = candidate_attribute[i]
            select_attributes.append(select_attribute)
            candidate_attribute.remove(select_attribute)
            accuracy = corrects[i]/float(len(test))
            accuracies.append(accuracy)
            break #avoid pick up two attribute in once, beacuse they have same accuracy
    print "attribute = ",select_attributes, "accuracy = ", accuracies, "c", candidate_attribute
    num_selec_attri +=1
    #print num_selec_attri
    acc_differ = accuracies[num_selec_attri] - accuracies[num_selec_attri-1]
